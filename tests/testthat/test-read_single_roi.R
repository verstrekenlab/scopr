...
test_that("read_single_roi fails when expected", {
  ...
  expect_error(fslscopr::read_single_roi(test_file,region_id = 1, columns=c("x", "z")), "Some of the requested columns are NOT available")
})
...
test_that("loading data works", {
  ...
  a <- fslscopr::read_single_roi(test_file,region_id = 1)
