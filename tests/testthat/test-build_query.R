...
test_that("build query works when time is supplied", {
  ...
  out <- fslscopr::build_query(dir, query)
...
test_that("build query works when only date supplied", {
  ...
  expect_warning(out <- fslscopr::build_query(dir, query), "Several files")
  expect_equal(nrow(out), 4)
...
test_that("missing results!", {
  ...
  expect_warning(out <- fslscopr::build_query(dir, query),   "No result for machine_name == E_0XXX")
  ...
  expect_warning(out <- fslscopr::build_query(dir, query),   "No result for machine_name == E_014, date == 2016-02-28")
  expect_equal(nrow(out), 2)
})
